...
def lb_five_on_five_pos(context,game_type="5v5",player_pk=None):
    ...
    if player_pk:
        ...
        for player in players:
            if season:
                pos_count = player.statline_set.filter(game__game_type=game_type,game__date__range=(season.start_date,season.end_date)).aggregate(Sum('off_pos'))
            else:
    ...
    dreb = helpers.per100_top_stat_players(game_type,'dreb',player_pk,excluded_pks,season_id=season_id)
    oreb = helpers.per100_top_stat_players(game_type,'oreb',player_pk,excluded_pks,season_id=season_id)  
    total_rebounds = helpers.per100_top_stat_players(game_type,'total_rebounds',player_pk,excluded_pks,season_id=season_id)
    asts = helpers.per100_top_stat_players(game_type,'asts',player_pk,excluded_pks,season_id=season_id)
    pot_ast = helpers.per100_top_stat_players(game_type,'pot_ast',player_pk,excluded_pks,season_id=season_id)
    stls = helpers.per100_top_stat_players(game_type,'stls',player_pk,excluded_pks,season_id=season_id)
    to = helpers.per100_top_stat_players(game_type,'to',player_pk,excluded_pks,season_id=season_id)
    points = helpers.per100_top_stat_players(game_type,'points',player_pk,excluded_pks,season_id=season_id)
    blk = helpers.per100_top_stat_players(game_type,'blk',player_pk,excluded_pks,season_id=season_id)
